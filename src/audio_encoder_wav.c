 ...
 *
 * Implements WAV (RIFF) format audio encoding for SlowFrame.
                            ...
                            const char *filename)
{
    ...
    if (!state->fp) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to open WAV output file: %s", filename);
...
{
    if (!state || !state->fp) {
        return SLOWFRAME_ERR_AUDIO_WAV;
    }
    ...
    if (fseek(state->fp, 0, SEEK_SET) != 0) {
        error_log(SLOWFRAME_ERR_AUDIO_WAV, "WAV header seek", "Failed to seek in output file");
        return SLOWFRAME_ERR_AUDIO_WAV;
    }

    // RIFF header
    if (fputs("RIFF", state->fp) == EOF) {
        error_log(SLOWFRAME_ERR_AUDIO_WAV, "WAV write", "Failed to write RIFF header");
        return SLOWFRAME_ERR_AUDIO_WAV;
    }

    // File size (little-endian)
    if (fputc((file_size & 0x000000ff), state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((file_size & 0x0000ff00) >> 8, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((file_size & 0x00ff0000) >> 16, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((file_size & 0xff000000) >> 24, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    if (fputs("WAVE", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // fmt sub-chunk
    if (fputs("fmt ", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Chunk size (16 for PCM)
    if (fputc(16, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Audio format (1 = PCM)
    if (fputc(1, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Channels
    if (fputc(state->channels, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Sample rate (little-endian)
    if (fputc((state->sample_rate & 0x000000ff), state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((state->sample_rate & 0x0000ff00) >> 8, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((state->sample_rate & 0x00ff0000) >> 16, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((state->sample_rate & 0xff000000) >> 24, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Byte rate (little-endian)
    if (fputc((byte_rate & 0x000000ff), state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((byte_rate & 0x0000ff00) >> 8, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((byte_rate & 0x00ff0000) >> 16, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((byte_rate & 0xff000000) >> 24, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Block align (little-endian)
    if (fputc((block_align & 0x00ff), state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((block_align & 0xff00) >> 8, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Bits per sample (little-endian)
    if (fputc((state->bit_depth & 0x00ff), state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((state->bit_depth & 0xff00) >> 8, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // data sub-chunk
    if (fputs("data", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;

    // Data size (little-endian)
    if (fputc((audio_data_size & 0x000000ff), state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((audio_data_size & 0x0000ff00) >> 8, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((audio_data_size & 0x00ff0000) >> 16, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
    if (fputc((audio_data_size & 0xff000000) >> 24, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_WAV;
                              ...
                              uint32_t sample_count)
{
    ...
    if (!buf) {
        error_log(SLOWFRAME_ERR_MEMORY_ALLOC, "Failed to allocate WAV write buffer (%zu bytes)", CHUNK_BYTES);
    ...
    while (samples_remaining > 0 && result == 0) {
        ...
        if (wrote != out_index) {
            error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to write WAV audio data (wrote %zu/%zu bytes)", wrote, out_index);
            result = SLOWFRAME_ERR_FILE_WRITE;
        }
...
static int wav_encoder_finish(AudioEncoderState *self)
{
    ...
    if (fseek(state->fp, 0, SEEK_END) != 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to seek to end of WAV file");
        return SLOWFRAME_ERR_FILE_WRITE;
    }

    long file_size = ftell(state->fp);
    if (file_size < 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to get WAV file size");
        return SLOWFRAME_ERR_FILE_WRITE;
    }
    ...
    if (audio_data_size < 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Invalid WAV file size: %ld", file_size);
        return SLOWFRAME_ERR_FILE_WRITE;
    }
    ...
    if (wav_encoder_write_header(state, sample_count) != 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to write WAV header");
        return SLOWFRAME_ERR_AUDIO_WAV;
    }

    // Flush and close file
    if (fflush(state->fp) != 0 || ferror(state->fp)) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to flush WAV output file");
        fclose(state->fp);
        state->fp = NULL;
        return SLOWFRAME_ERR_FILE_WRITE;
    }
    ...
    if (fclose(state->fp) != 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to close WAV output file");
        return SLOWFRAME_ERR_FILE_WRITE;
    }
