...
static int wav_encoder_write_header(WavEncoderState *state, uint32_t audio_sample_count)
{
    ...
    if (fputc((file_size & 0x0000ff00) >> 8, state->fp) == EOF) return -1;
    if (fputc((file_size & 0x00ff0000) >> 16, state->fp) == EOF) return -1;
    if (fputc((file_size & 0xff000000) >> 24, state->fp) == EOF) return -1;

    if (fputs("WAVE", state->fp) == EOF) return -1;

    // fmt sub-chunk
    if (fputs("fmt ", state->fp) == EOF) return -1;

    // Chunk size (16 for PCM)
    if (fputc(16, state->fp) == EOF) return -1;
    if (fputc(0, state->fp) == EOF) return -1;
    if (fputc(0, state->fp) == EOF) return -1;
    if (fputc(0, state->fp) == EOF) return -1;

    // Audio format (1 = PCM)
    if (fputc(1, state->fp) == EOF) return -1;
    if (fputc(0, state->fp) == EOF) return -1;
    ...
    if (fputc((state->sample_rate & 0x000000ff), state->fp) == EOF) return -1;
    if (fputc((state->sample_rate & 0x0000ff00) >> 8, state->fp) == EOF) return -1;
    if (fputc((state->sample_rate & 0x00ff0000) >> 16, state->fp) == EOF) return -1;
    if (fputc((state->sample_rate & 0xff000000) >> 24, state->fp) == EOF) return -1;

    // Byte rate (little-endian)
    if (fputc((byte_rate & 0x000000ff), state->fp) == EOF) return -1;
    if (fputc((byte_rate & 0x0000ff00) >> 8, state->fp) == EOF) return -1;
    if (fputc((byte_rate & 0x00ff0000) >> 16, state->fp) == EOF) return -1;
    if (fputc((byte_rate & 0xff000000) >> 24, state->fp) == EOF) return -1;

    // Block align (little-endian)
    if (fputc((block_align & 0x00ff), state->fp) == EOF) return -1;
    if (fputc((block_align & 0xff00) >> 8, state->fp) == EOF) return -1;

    // Bits per sample (little-endian)
    if (fputc((state->bit_depth & 0x00ff), state->fp) == EOF) return -1;
    if (fputc((state->bit_depth & 0xff00) >> 8, state->fp) == EOF) return -1;

    // data sub-chunk
    if (fputs("data", state->fp) == EOF) return -1;

    // Data size (little-endian)
    if (fputc((audio_data_size & 0x000000ff), state->fp) == EOF) return -1;
    if (fputc((audio_data_size & 0x0000ff00) >> 8, state->fp) == EOF) return -1;
    if (fputc((audio_data_size & 0x00ff0000) >> 16, state->fp) == EOF) return -1;
    if (fputc((audio_data_size & 0xff000000) >> 24, state->fp) == EOF) return -1;
                              ...
                              uint32_t sample_count)
{
    ...
    while (samples_remaining > 0 && result == 0) {
        ...
        if (wrote != out_index) {
            fprintf(stderr, "[ERROR] write failed while writing WAV audio\n");
            perror("fwrite");
            result = -1;
        }
