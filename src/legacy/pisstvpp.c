...
static void print_usage(const char *prog) {
    fprintf(stderr, "Usage: %s [-p m1|m2|s1|s2|sdx|r36] [-r rate] <image>\n", prog);
    fprintf(stderr, "\n");
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "  -p  SSTV protocol (default: m1)\n");
    fprintf(stderr, "  -r  Sample rate in Hz (default: %d, max: %d)\n", RATE, MAXRATE);
    fprintf(stderr, "  -h  Show this help\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Notes:\n");
    fprintf(stderr, "  - Input image must be at least 320x256 for Martin/Scottie or 320x240 for Robot 36.\n");
    fprintf(stderr, "  - Output file will be written alongside the input with .wav or .aiff extension.\n");
}
...
int main(int argc, char *argv[]) {
    ...
    g_rate = RATE;
    while ((option = getopt(argc, argv, "r:p:h")) != -1) {
        switch (option) {
            case 'r':
                g_rate = (atoi(optarg));
                break;
    ...
    if (g_rate <= 0 || g_rate > MAXRATE) {
        fprintf(stderr, "Invalid sample rate: %d (max %d)\n", g_rate, MAXRATE);
        return 1;
    }
    ...
    uint8_t ft ; 
    char inputfile[256], outputfile[512] ;
...
#endif
#ifdef AUDIO_WAV    
    strncat( outputfile , "wav", sizeof(outputfile) - strlen(outputfile) - 1 ) ;
#endif
    ...
    int img_w = gdImageSX(g_imgp);
    int img_h = gdImageSY(g_imgp);
    int min_h = (g_protocol == 8) ? 240 : 256;
    if (img_w < 320 || img_h < min_h) {
        fprintf(stderr, "Input image too small: %dx%d (need >=320x%d)\n", img_w, img_h, min_h);
...
uint8_t filetype( char *filename ) {
    ...
    {
        const char *magic_str = magic_file(m, filename);
        if (!magic_str) {
        ...
        strncpy(m_str, magic_str, MAGIC_CNT+1) ;
...
void playtone( uint16_t tonefreq , double tonedur ) {
    ...
    if (g_samples + tonesamples >= MAXSAMPLES) {
        fprintf(stderr, "Exceeded maximum audio buffer (%u samples).\n", MAXSAMPLES);
        exit(2);
    }
...
void writefile_aiff () {
    ...
    // NOTE: Loop corrected from i<=g_samples to i<g_samples to prevent buffer overrun
