...
// IMPORTANT: This file contains C++ code and must be compiled with a C++ compiler.
// Despite the .c extension, g++ is required: g++ -O3 -o pifm pifm_sstv.c
...
//
// NOTE: This code is Raspberry Pi specific and requires:
// - Direct /dev/mem access (requires root)
// - BCM2835+ hardware (Raspberry Pi 1-4)
// - Access to /proc/self/pagemap (for page translation)
...
#include <fcntl.h>
#include <assert.h>
/* malloc.h is Linux-specific; stdlib.h is portable */
#ifndef __APPLE__
#include <malloc.h>
#endif
#include <sys/mman.h>
...
#define ACCESS(base) *(volatile int*)((uintptr_t)allof7e+base-0x7e000000)
...
void getRealMemPage(void** vAddr, void** pAddr) {
    ...
    *vAddr = a;  // yay - we know the virtual address
    ...
    if (fp < 0) {
        perror("ERROR: open /proc/self/pagemap");
        munlock(a, 4096);
        free(a);
        *vAddr = NULL;
        *pAddr = NULL;
    ...
    off_t offset = ((uintptr_t)a)/4096*8;
    if (lseek(fp, offset, SEEK_SET) < 0) {
        perror("ERROR: lseek pagemap");
        close(fp);
        munlock(a, 4096);
        free(a);
        *vAddr = NULL;
    ...
    if (bytes_read != sizeof(frameinfo)) {
        perror("ERROR: read pagemap");
        munlock(a, 4096);
        free(a);
        *vAddr = NULL;
        *pAddr = NULL;
    ...
    // Use uintptr_t for correct pointer arithmetic on both 32-bit and 64-bit systems
...
void setup_fm(int state)
{
    ...
    if ((mem_fd = open("/dev/mem", O_RDWR|O_SYNC) ) < 0) {
        fprintf(stderr, "ERROR: can't open /dev/mem (require root?)\n");
        exit (-1);
    }
    ...
    struct GPCTL setupword = {6/*SRC*/, 1, 0, 0, 0, (char)state, 0x5a};
...
//        clocksPerSample = 22500.0 / rate * 1373.5;  // for timing, determined by experiment
    ...
    void consume(float* data, int num) {
        for (int i=0; i<num; i++) {
            ...
            while( (ACCESS(DMABASE + 0x04 /* CurBlock*/) & ~ 0x7F) ==  (unsigned int)(uintptr_t)(instrs[bufPtr].p)) {
            ...
            ((struct CB*)(instrs[bufPtr].v))->SOURCE_AD = (unsigned int)(uintptr_t)constPage.p + 2048 + intval*4 - 4 ;
            bufPtr++;
            
            // Create DMA command to delay using serializer module for suitable time.
            ((struct CB*)(instrs[bufPtr].v))->TXFR_LEN = (unsigned int)((int)timeErr-fracval);
            bufPtr++;
...
void playWav(char* filename, float samplerate, float bandwidth, float timing_correction, bool stereo)
{
    ...
    int readBytes;
    while ((readBytes = read(fp, &data, sizeof(data))) > 0) {
...
void unSetupDMA(){
    fprintf(stderr, "Shutting down DMA...\n");
...
void setupDMA( float centerFreq ){
  ...
  signal (SIGQUIT, handSig);
   ...
   if (!constPage.v) {
       fprintf(stderr, "ERROR: Failed to allocate constant page\n");
       exit(-1);
   }
   ...
   while (instrCnt<BUFFERINSTRUCTIONS) {
     ...
     if (!instrPage.v) {
         fprintf(stderr, "ERROR: Failed to allocate instruction page %d\n", instrCnt);
         exit(-1);
     }
     ...
     for (int i=0; i<4096/sizeof(struct CB); i++) {
       ...
       instrs[instrCnt].p = (void*)((uintptr_t)instrPage.p + sizeof(struct CB)*i);
       instr0->SOURCE_AD = (unsigned int)(uintptr_t)constPage.p+2048;
       ...
       if (instrCnt!=0) ((struct CB*)(instrs[instrCnt-1].v))->NEXTCONBK = (unsigned int)(uintptr_t)instrs[instrCnt].p;
       instr0++;
       instrCnt++;
     }
   }
   ((struct CB*)(instrs[BUFFERINSTRUCTIONS-1].v))->NEXTCONBK = (unsigned int)(uintptr_t)instrs[0].p;
   ...
   DMA0->CONBLK_AD=0; 
   DMA0->TI=0; 
   DMA0->CONBLK_AD = (unsigned int)(uintptr_t)(instrPage.p);
   DMA0->CS =(1<<0)|(255 <<16);  // enable bit = 0, clear end flag = 1, prio=19-16
   
   setup_complete = 1;  // Mark that DMA setup is complete
}
...
int main(int argc, char **argv)
{
    ...
    playWav(argv[1], argc > 3 ? atof(argv[3]) : DEFAULTRATE,
            argc > 4 ? atof(argv[4]) : DEFAULTBANDWIDTH,
            argc > 5 ? atof(argv[5]) : DEFAULTTIMING,
            argc > 6);
