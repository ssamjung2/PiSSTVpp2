 ...
 *
 * Implements AIFF (Audio Interchange File Format) encoding for SlowFrame.
                             ...
                             const char *filename)
{
    ...
    if (!state->fp) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to open AIFF output file: %s", filename);
...
{
    if (!state || !state->fp) {
        return SLOWFRAME_ERR_AUDIO_AIFF;
    }
    ...
    if (fseek(state->fp, 0, SEEK_SET) != 0) {
        error_log(SLOWFRAME_ERR_AUDIO_AIFF, "AIFF header seek", "Failed to seek in output file");
        return SLOWFRAME_ERR_AUDIO_AIFF;
    }

    // FORM chunk
    if (fputs("FORM", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // Form size (big-endian)
    if (fputc((form_size >> 24) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc((form_size >> 16) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc((form_size >> 8) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(form_size & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    if (fputs("AIFF", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // COMM chunk (Common)
    if (fputs("COMM", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // COMM chunk size (always 18 for PCM)
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(18, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // Number of channels (big-endian)
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(state->channels, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // Number of sample frames (big-endian)
    if (fputc((audio_sample_count >> 24) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc((audio_sample_count >> 16) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc((audio_sample_count >> 8) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(audio_sample_count & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // Bits per sample (big-endian)
    if (fputc(0, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(state->bit_depth, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // Sample rate (80-bit extended precision format)
    uint8_t ext_rate[10];
    sample_rate_to_extended(state->sample_rate, ext_rate);
    for (int i = 0; i < 10; i++) {
        if (fputc(ext_rate[i], state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    }

    // SSND chunk (Sound Data)
    if (fputs("SSND", state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;

    // SSND chunk size (big-endian)
    uint32_t ssnd_size = 8 + audio_data_size;
    if (fputc((ssnd_size >> 24) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc((ssnd_size >> 16) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc((ssnd_size >> 8) & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
    if (fputc(ssnd_size & 0xff, state->fp) == EOF) return SLOWFRAME_ERR_AUDIO_AIFF;
                               ...
                               uint32_t sample_count)
{
    ...
    if (!buf) {
        error_log(SLOWFRAME_ERR_MEMORY_ALLOC, "Failed to allocate AIFF write buffer (%zu bytes)", CHUNK_BYTES);
    ...
    while (samples_remaining > 0 && result == 0) {
        ...
        if (wrote != out_index) {
            error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to write AIFF audio data (wrote %zu/%zu bytes)", wrote, out_index);
...
static int aiff_encoder_finish(AudioEncoderState *self)
{
    ...
    if (fseek(state->fp, 0, SEEK_END) != 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to seek to end of AIFF file");
        return SLOWFRAME_ERR_FILE_WRITE;
    }

    long file_size = ftell(state->fp);
    if (file_size < 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to get AIFF file size");
        return SLOWFRAME_ERR_FILE_WRITE;
    }
    ...
    if (audio_data_size < 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Invalid AIFF file size: %ld", file_size);
        return SLOWFRAME_ERR_FILE_WRITE;
    }
    ...
    if (aiff_encoder_write_header(state, sample_count) != 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to write AIFF header");
        return SLOWFRAME_ERR_AUDIO_AIFF;
    }

    // Flush and close file
    if (fflush(state->fp) != 0 || ferror(state->fp)) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to flush AIFF output file");
        fclose(state->fp);
        state->fp = NULL;
        return SLOWFRAME_ERR_FILE_WRITE;
    }
    ...
    if (fclose(state->fp) != 0) {
        error_log(SLOWFRAME_ERR_FILE_WRITE, "Failed to close AIFF output file");
        return SLOWFRAME_ERR_FILE_WRITE;
    }
